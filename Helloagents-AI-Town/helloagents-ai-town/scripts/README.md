...
**信号**:
- `chat_response_received(npc_name, message, emotion)` - 收到对话回复和NPC情绪
...
func _on_response(npc_name, message, emotion):
    print(npc_name + ": " + message)
    print("情绪: " + str(emotion.get("label", "平静")))
```
...
api_client.gd: _on_chat_request_completed()
    ↓
发出信号: chat_response_received(npc_name, message, emotion)
    ↓
