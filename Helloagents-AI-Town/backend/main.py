...
from models import (
    ...
    TaskAcceptResponse, TaskEvent, TaskInfo, TaskListResponse,
    PlayerProfileResponse, ProfileClearResponse, ProfileItemStatusUpdate,
    PlayerProfileItem, TimeHabitInfo
)
...
async def root():
    ...
    return {
        ...
        "version": settings.API_VERSION,
        "status": "running",
            "features": ["AI对话", "NPC记忆系统", "好感度系统", "NPC情绪系统", "批量状态更新", "NPC任务系统", "玩家画像系统"],
        "endpoints": {
            "docs": "/docs",
            "chat": "/chat",
            "player_profile": "/players/{player_id}/profile",
            "tasks": "/tasks",
            "task_accept": "/tasks/{task_id}/accept",
            "npcs": "/npcs",
...
async def chat_with_npc(request: ChatRequest):
    ...
    try:
        # 调用NPC Agent处理对话
        chat_result = npc_mgr.chat_with_events(
            request.npc_name,
            request.message,
            request.player_id
        )
        ...
        return ChatResponse(
            ...
            task_events=[TaskEvent(**event) for event in chat_result.get("task_events", [])],
            emotion=chat_result.get("emotion") or npc_info.get("emotion"),
            affinity=float(chat_result.get("affinity", npc_mgr._get_affinity_value(request.npc_name, request.player_id))),
            success=True
        )
...
async def list_tasks(player_id: str = "player"):
    """获取当前玩家的NPC任务列表"""
    npc_mgr, _ = get_managers()

    try:
        tasks_data = npc_mgr.get_tasks(player_id)
        tasks = [TaskInfo(**task) for task in tasks_data]
...
@app.post("/tasks/{task_id}/accept", response_model=TaskAcceptResponse)
async def accept_task(task_id: str, player_id: str = "player"):
    """接受指定NPC任务"""
    npc_mgr, _ = get_managers()

    try:
        result = npc_mgr.accept_task(task_id, player_id)
        if not result["success"] and result["task"] is None:
            raise HTTPException(
                status_code=404,
        ...
        task = TaskInfo(**result["task"]) if result.get("task") else None
...
async def get_player_profile(player_id: str):
    ...
    try:
        profile = npc_mgr.get_player_profile(player_id)
        return PlayerProfileResponse(
            player_id=profile["player_id"],
            items=[PlayerProfileItem(**item) for item in profile.get("items", [])],
            time_habits=[TimeHabitInfo(**item) for item in profile.get("time_habits", [])],
            summary=profile.get("summary", "")
        )
    except Exception as e:
        raise HTTPException(
            status_code=500,
            detail=f"获取玩家画像失败: {str(e)}"
        )

@app.delete("/players/{player_id}/profile", response_model=ProfileClearResponse)
async def clear_player_profile(player_id: str):
    """清空玩家画像"""
    npc_mgr, _ = get_managers()

    try:
        result = npc_mgr.clear_player_profile(player_id)
        return ProfileClearResponse(
            player_id=player_id,
        ...
        raise HTTPException(
            status_code=500,
            detail=f"清空玩家画像失败: {str(e)}"
        )

@app.patch("/players/{player_id}/profile/items/{item_id}")
async def update_player_profile_item_status(
    player_id: str,
    item_id: int,
    request: ProfileItemStatusUpdate,
):
    """更新玩家画像项状态"""
    npc_mgr, _ = get_managers()

    try:
        item = npc_mgr.update_player_profile_item_status(player_id, item_id, request.status)
        if item.get("category") == "time":
            return TimeHabitInfo(**item)
        return PlayerProfileItem(**item)
    except KeyError:
        raise HTTPException(
            status_code=404,
            detail=f"画像项 '{item_id}' 不存在"
        )
