...
from player_profile_manager import PlayerProfileManager, get_player_profile_manager
from logger import (
...
class NPCAgentManager:
    ...
    def __init__(self):
        ...
        self.task_manager = get_task_manager()  # ⭐ NPC任务管理器
        self.player_profile_manager: PlayerProfileManager = get_player_profile_manager()
    ...
    def chat(self, npc_name: str, message: str, player_id: str = "player") -> str:
        """与指定NPC对话 (支持记忆功能、好感度系统和任务系统)"""
        return self.chat_with_events(npc_name, message, player_id)["message"]

    def chat_with_events(self, npc_name: str, message: str, player_id: str = "player") -> Dict[str, Any]:
        """与指定NPC对话,返回NPC回复和任务事件"""
        ...
        if agent is None:
            ...
            self._record_player_profile_interaction(
                npc_name=npc_name,
            ...
            return {
                ...
                "task_events": [],
                "emotion": self._get_emotion_value(npc_name, player_id)
            }
        ...
        try:
            ...
            # ⭐ 0. 记录玩家与NPC接触,用于跨NPC任务进度
            self.task_manager.record_npc_contact(player_id, npc_name)
            ...
            if self.relationship_manager:
                ...
                emotion_context = self.relationship_manager.get_emotion_prompt_context(npc_name, player_id)
...
"""
            ...
            task_context = self.task_manager.build_prompt_context(
                npc_name=npc_name,
                player_id=player_id,
                get_affinity=self._get_affinity_value
            )
            if task_context:
                enhanced_message += task_context
            ...
            # ⭐ 7. 根据本轮对话处理任务接受/完成
            task_events = self.task_manager.process_dialogue_after_response(
                npc_name=npc_name,
                player_message=message,
                player_id=player_id,
                get_affinity=self._get_affinity_value
            ...
            if task_events:
                ...
                response = self._append_task_event_messages(response, task_events)

            # ⭐ 8. 更新玩家画像。画像失败不影响聊天主流程。
            self._record_player_profile_interaction(
                npc_name=npc_name,
                player_message=message,
                player_id=player_id,
            ...
            return {
                "message": response,
                "task_events": task_events,
                "emotion": affinity_result.get("emotion", self._get_emotion_value(npc_name, player_id)),
                "affinity": affinity_result.get("new_affinity", affinity_result.get("affinity", self._get_affinity_value(npc_name, player_id)))
            }
            ...
            return {
                ...
                "task_events": [],
                "emotion": self._get_emotion_value(npc_name, player_id),
                "affinity": self._get_affinity_value(npc_name, player_id)
            }
    ...
    def _save_conversation_to_memory(
        ...
        memory_manager.add_memory(
            ...
            memory_type="episodic",  # 直接写入情景记忆,确保进入SQLite+向量库可被检索
            importance=0.5,  # 中等重要性
            auto_classify=False,  # 关闭自动分类,否则对话会被按关键词重分类到working内存
            metadata={
                "speaker": "player",
                "player_id": player_id,
        ...
        # 同时保留到工作记忆(用于短期上下文,重启后清空)
        try:
            memory_manager.add_memory(
                content=f"玩家说: {player_message}",
                memory_type="working",
                importance=0.5,
                auto_classify=False,  # 显式指定类型,避免被自动分类改写
                metadata={
                    "speaker": "player",
                    "session_id": player_id,
                    "timestamp": current_time.isoformat(),
    ...
    def _apply_task_events(
        ...
        """Apply task side effects such as reward and completion memory."""
        memory_manager = self.memories.get(npc_name)
        ...
        for event in task_events:
            ...
            reward = float(event.get("reward_affinity", 0.0))
            if self.relationship_manager:
                current_affinity = self.relationship_manager.get_affinity(npc_name, player_id)
                new_affinity = min(100.0, current_affinity + reward)
                self.relationship_manager.set_affinity(npc_name, new_affinity, player_id)
                event["new_affinity"] = new_affinity
                print(f"  🎁 任务奖励: {npc_name}好感度 +{reward:.0f} -> {new_affinity:.0f}")

            if memory_manager:
                self._save_task_completion_to_memory(
                    memory_manager=memory_manager,
                    npc_name=npc_name,
    ...
    def _save_task_completion_to_memory(
        self,
        memory_manager: MemoryManager,
        npc_name: str,
        player_message: str,
        ...
        """Save completed task as high-importance episodic memory."""
        current_time = datetime.now()
        content = (
            f"玩家完成了任务「{task_event.get('title', '')}」。"
            f"玩家的关键反馈: {player_message}"
        )
        ...
        memory_manager.add_memory(
            ...
            metadata={
                ...
                "context": {
                    ...
                    "task_title": task_event.get("title"),
                    "reward_affinity": task_event.get("reward_affinity", 0.0),
                    "new_affinity": task_event.get("new_affinity")
                }
            },
            auto_classify=False
        )
        print(f"  💾 任务完成事件已保存到{npc_name}的长期记忆中")
    ...
    def _build_player_profile_context(
        ...
        """Build safe player profile prompt context."""
        try:
            return self.player_profile_manager.build_prompt_context(
                player_id=player_id,
                npc_name=npc_name,
                current_message=player_message,
            )
        except Exception as e:
            print(f"⚠️  玩家画像上下文生成失败,已跳过: {e}")
            return ""

    def _record_player_profile_interaction(
        self,
        npc_name: str,
        player_message: str,
        player_id: str,
        ...
        """Update player profile from chat-side signals without blocking chat."""
        try:
            result = self.player_profile_manager.record_chat_message(
                player_id=player_id,
                npc_name=npc_name,
                message=player_message,
            )
            task_updates = self.player_profile_manager.record_task_events(
                player_id=player_id,
                npc_name=npc_name,
                task_events=task_events,
            )
            relationship_update = self.player_profile_manager.record_relationship_event(
                player_id=player_id,
                npc_name=npc_name,
                affinity_result=affinity_result,
            )
            signal_count = len(result.get("signals", [])) + len(task_updates)
            if relationship_update:
                signal_count += 1
            print(f"  👤 玩家画像已更新 (signals={signal_count})")
        except Exception as e:
            print(f"⚠️  玩家画像更新失败,已跳过: {e}")

    def _append_task_event_messages(self, response: str, task_events: List[Dict[str, Any]]) -> str:
        """Append deterministic task notices to the NPC response."""
        notices = []
        for event in task_events:
            if event.get("event_type") == "accepted":
                notices.append(f"【任务】已接受: {event.get('title', '')}")
            elif event.get("event_type") == "completed":
                reward = float(event.get("reward_affinity", 0.0))
                notice = f"【任务完成】{event.get('title', '')}, 好感度 +{reward:.0f}"
                if event.get("new_affinity") is not None:
                    notice += f" (当前 {event['new_affinity']:.0f})"
    ...
    def _get_affinity_value(self, npc_name: str, player_id: str = "player") -> float:
        """Return affinity score used by task unlock rules."""
        if not self.relationship_manager:
            return 50.0
        return self.relationship_manager.get_affinity(npc_name, player_id)
    ...
    def get_npc_info(self, npc_name: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "emotion": self._get_emotion_value(npc_name)
        }
    ...
    def _get_emotion_value(self, npc_name: str, player_id: str = "player") -> Optional[Dict[str, Any]]:
        """Return current NPC emotion state if the relationship manager exists."""
        if not self.relationship_manager:
            return None
        return self.relationship_manager.get_emotion_state(npc_name, player_id)
    ...
    def get_tasks(self, player_id: str = "player") -> List[Dict[str, Any]]:
        """获取当前玩家的所有任务"""
        return self.task_manager.get_tasks(player_id, self._get_affinity_value)

    def accept_task(self, task_id: str, player_id: str = "player") -> Dict[str, Any]:
        """接受任务"""
        result = self.task_manager.accept_task(task_id, player_id, self._get_affinity_value)
        task = result.get("task")
        if result.get("success") and task:
            try:
                self.player_profile_manager.record_task_events(
                    player_id=player_id,
                    ...
                    task_events=[{
                        ...
                        "npc_name": task["npc_name"],
                        "title": task["title"],
                        "message": result.get("reason", "任务已接受"),
                    }],
                )
            except Exception as e:
                print(f"⚠️  任务画像事件记录失败,已跳过: {e}")
        return result

    def get_player_profile(self, player_id: str = "player") -> Dict[str, Any]:
        """获取玩家画像"""
        return self.player_profile_manager.get_profile(player_id)

    def clear_player_profile(self, player_id: str = "player") -> Dict[str, int]:
        """清空玩家画像"""
        return self.player_profile_manager.clear_profile(player_id)

    def update_player_profile_item_status(
        self,
        player_id: str,
        item_id: int,
        status: str,
        ...
        """更新画像项状态"""
        return self.player_profile_manager.update_item_status(player_id, item_id, status)
